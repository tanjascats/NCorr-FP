...
from utils import *
from NCorrFP.NBNN import CategoricalNeighbourhood

warnings.filterwarnings("ignore")

scheme_params = {'gamma': 3, 'xi': 2}
n_experiments = 1  # number of runs withing one single fingerprinted data set
n_fingerprint_experiments = 1  # how many times do I want to fingerprint the data set
fail = 0
score_diff_total = 0
start = time.time()
random.seed(333)
...
for fp_exp in range(n_fingerprint_experiments):
    ...
    for exp in range(n_experiments):
        ...
        model = LogisticRegression(random_state=0, solver=best_params['solver'], C=best_params['C'])
